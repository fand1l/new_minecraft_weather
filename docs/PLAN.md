...
серверний менеджер, п'ять серверних міксинів, обидва дерева команд. M10 звірено з реальним
тілом ванільного `tickThunder` — див. `VERIFIED-API-26.2.md`; відкритих імен більше немає.
...
|---|---|---|
| Рендер погоди | `net.minecraft.client.renderer.WeatherEffectRenderer` | ✅ тіла прочитані; M5 виявився неможливим, M6 звузився до `renderInstances` |
...
**Рендер: `RenderStateDataKey` і `LevelRenderEvents` не потрібні — і власний draw теж.**
Після читання джерел M6 звузився з `render` до приватного `renderInstances`: уся GPU-частина
(пайплайн, таргет, буфери, юніформи, draw-виклики) лишається ванільною, а ми лише пишемо
вершини зі зсунутою верхньою парою. Вітер читаємо зі свого клієнтського стану, а не веземо
в render-state. Раніше тут перелічувались `RenderPipelines.WEATHER_DEPTH_WRITE` /
...
| `WeatherGridPayload` | вхід, зміна виміру, телепорт далі за екстент | ~24 КБ разово (55×55 вузлів × 8 Б). |
| `WeatherGridDeltaPayload` | 40 тіків (2 с) | Стани зон міняються раз на ~ігрову добу, тому змінюється мало вузлів. Типова дельта при русі гравця — один новий ряд, `55 × 8 = 440 B`. Клієнт інтерполює в часі між двома кадрами, тож 2 с не помітні. |

**Оцінка трафіку:** усталений режим — **~220 Б/с на гравця** (440 B / 2 c), пік при
спринті через межу зони — до ~3 КБ/с. Вимога «трафік на гравця мінімальний» виконана:
повна сітка на вході менша за один чанк.

**Обмеження, яке треба поважати** (переформульовано після рев'ю — стара редакція
в'язала смугу до кроку сітки й через це робила зону радіусом 64 неможливою):
`radius_min >= 4 × grid.step`. При дефолтах `64 >= 4 × 16` сходиться рівно.
Сама смуга бленду тепер похідна від радіуса зони, а не від сітки — див.
«Розподіл розмірів зон і смуга бленду». `ConfigManager` валідує цей єдиний інваріант
і лається в лог при порушенні. Хочеш різкішу стіну дощу — зменшуй `grid.step`,
але пам'ятай, що він же задає мінімально допустимий радіус зони.
...
| ~~**M5**~~ | ~~`WeatherEffectRenderer#extractRenderState`~~ | — | **Викреслено після читання джерел.** `renderInstances` індексує таблицю розмірів колонок як `[(dz+16)*32 + dx+16]` — масив 32×32 без жодного клемпа. Радіус понад 15 блоків не «розширює» дощ, а кладе рендер-тред. Ванільна опція `weatherRadius` лишається як є, поле з конфігу прибрано. |
| **M6** | `WeatherEffectRenderer#renderInstances` (приватний) | `@Inject HEAD cancellable` | **Нахил опадів за вітром.** Запис `ColumnInstance` поля нахилу не має, тож зсув верхньої пари вершин можна зробити лише там, де вершини пишуться. Ціль звужено з `render` до приватного помічника після читання джерел: уся GPU-частина (пайплайн, таргет, буфери, юніформи, два draw-виклики) лишається ванільною. **Ні власного пайплайна, ні шейдера, ні сирого графічного виклику в моді немає.** При штилі міксин узагалі не перехоплює — малює ваніль. |
...
| **M9** | `ServerLevel#tickPrecipitation(BlockPos)` | `@Inject HEAD cancellable`, заміна тіла | **Обхід M2:** метод питає біом напряму, а не `precipitationAt`, тож без гейта казан і сніг накопичувались би по всій карті, поки `isRaining()` глобально true. Тіло відтворено з джерел 26.2, бо намерзання льоду стоїть **вище** ванільної перевірки дощу: попередня редакція скасовувала метод цілком і тихо вимикала замерзання води скрізь, де не йшов дощ. Тепер заморозка ванільна й безумовна, локально гейтиться лише дощова половина — вона ж множиться на рівень опадів (казан, сніг). |
| **M11** | `CloudRenderer#render` | `@ModifyVariable` на аргумент кольору | **Вісь хмарності.** У ванілі хмари — один шар без поняття щільності, вимикати чи згущувати нема чого. Але `LevelRenderer` уже гейтить весь прохід на `ARGB.alpha(cloudColor) > 0`, тобто альфа кольору — єдина ручка, яку ваніль сама поважає. Масштабуємо її: ясно → 0, суцільна хмарність → ванільне значення. Ні своєї геометрії, ні свого проходу. Масштабуємо, а не перезаписуємо, бо у вхідному кольорі вже сидить ванільне затемнення від часу доби й від дощу — а дощ у нас локальний. |
| **M10** | `ServerLevel#tickThunder(LevelChunk)` | `@Inject HEAD cancellable`, повна заміна тіла (~25 рядків) | **Обхід дощових гейтів заради сухої грози.** Ванільний `isRainingAt(pos)` після M2 дає `false` без опадів, тому суха гроза не била б узагалі. Гейт міняємо на нашу вісь грози, частоту — під `WEAK`/`NORMAL`. Прогрузка чанків лишається ванільною задарма. Решту (ґеймрул `SPAWN_MOBS`, кидок на складність, громовідвід, пастку зі скелетним конем, `snapTo`, `setVisualOnly`) відтворено **з реального тіла 26.2**, включно з порядком `&&`, бо він визначає, скільки значень з'їдається з генератора. |
...
### Стіна дощу: чому її немає і що виявилось замість неї

Позиція не змінилась: ванільним рендером опадів стіни на горизонті не зробити. Причини
нижче в цьому розділі — таблиця розмірів колонок обмежена 15 блоками, а перебір при 512
блоках дає мільйон колонок на кадр.

**Але після читання `CloudRenderer` з'явився інший шлях до тієї самої мети.** Хмари в 26.2 —
не суцільний шар, а **сітка комірок**, побудована з `textures/environment/clouds.png`:

```java
public record TextureData(long[] cells, int width, int height);
private static boolean isCellEmpty(final int color);
private void tryBuildCell(...);       // рядок 251 — вирішує, чи існує комірка
public void markForRebuild();         // публічний
private static final float CELL_SIZE_IN_BLOCKS = 12.0F;
```

Якщо в `tryBuildCell` можна скасувати комірку, то хмарність стає **просторовою**: комірка
питає нашу сітку погоди у своїй світовій точці й зникає там, де ясно. Наслідки:

1. **Щільність, а не прозорість.** Рівно те, чого бракувало: `few` — рідкі комірки,
   `overcast` — суцільні. Без власного пайплайна, без шейдера, без своєї геометрії — той
   самий підхід, що в M6.
2. **Далеке видно.** Шар хмар малюється на всю `cloudRange`, тобто набагато далі за 15
   блоків опадів. Темна маса над штормом за кілометр — це і є та ознака, що «там негода»,
   і в реальному житті грозу здалеку впізнають саме так, а не по видимій стіні дощу.

Тобто дальню ознаку локальності дає **шар хмар**, а завіса опадів лишається ближньою. Це
не те, що просило ТЗ дослівно, але це та сама мета за незрівнянно меншого ризику.

Що для цього треба прочитати: сигнатуру `tryBuildCell` і тіло `buildMesh` — щоб знати, чи
є там світові координати комірки, і як змусити меш перебудуватись при зміні погоди
(`markForRebuild` публічний, але треба зрозуміти, що саме його тригерить).

### Чого немає: стіна дощу на всю дальність промальовування

ТЗ вимагає, щоб межа зони була **видима здалеку** — стіна дощу на дальності промальовування,
а не «вмикання» опадів навколо гравця. Після читання джерел 26.2 кажу прямо: **ванільним
рендером погоди цього зробити не можна**, і ось чому саме.

1. `renderInstances` бере ширину кожної колонки з таблиці `columnSizeX[(dz+16)*32 + dx+16]`.
   Це масив 32×32 без клемпа, тобто радіус фізично обмежений 15 блоками.
2. Навіть якби обмеження не було, `extractRenderState` — це подвійний цикл по всіх колонках
   у квадраті радіуса. При 15 це 961 колонка на кадр; при 512 (дальність промальовування
   32 чанки) — **1 050 625 колонок на кадр**, кожна з `ColumnInstance` і зверненням до
   хітмапу. Це не «просадка FPS», це слайдшоу.

Тобто стіна дощу на горизонті — це не налаштування ванільного рендера, а **окрема підсистема**:
кілька великих напівпрозорих полотнищ уздовж межі зони, з власним пайплайном і сортуванням,
зі своїм LOD. Це найбільший і найризикованіший шматок у всьому проєкті, і я не буду вдавати,
що він виходить із підміни одного числа — саме так це виглядало в попередній редакції плану.

**Що з локальності видно вже зараз, без цієї підсистеми:**

- дизеринг краю (M4) — щільність колонок спадає плавно, тож у межах ванільного радіуса межа
  читається як завіса, що розчиняється, а не як стіна з рубленим краєм;
- туман (M7) і затемнення неба (M3) реагують на погоду **в точці камери**, тож наближення
  до шторму відчувається ще до першої краплі;
- висотна межа працює повністю: з-під хмар можна вилетіти вгору й вийти зі шторму.

**Що я пропоную далі.** Винести стіну дощу в окремий крок після того, як усе інше працює
й перевірене в грі. Тоді її можна робити з нуля, знаючи, що модель, мережа й геймплей уже
не змінюються, а не змішувати найризикованіший рендер із рештою.
...
| **Рендер опадів на всю дальність** | ❌ **Не зроблено, і ванільним рендером зробити не можна** — див. окремий розділ нижче | Чесна відмова замість ілюзії. Що працює: дизеринг краю (M4) робить межу зони видимою в межах ванільного радіуса, а туман і затемнення неба реагують на локальну погоду ще до того, як побачиш дощ. |
| **Оверрайд команди тоне в середньому** | ❌ Сталося в грі: при 15 зонах `set thunder normal` приходив як 0.33, `set wind gale` як 0.09 — нижче порогу GALE, тож ні нахилу, ні фізики | ✅ Виправлено: блендер відкидає природні зони, якщо точку накриває хоч одна командна, а другий `set` замінює перший замість накопичення. Закрито перевірками [36] і [37] у харнесі. Джерело помилки — коментар, який стверджував правило, якого в коді не було. |
| **Роздільність сітки vs розмір зони** | Смуга бленду ширша за пів-радіуса — і зона не набирає повної інтенсивності взагалі | Смуга похідна від радіуса: `clamp(radius · 0.35, 2 · grid.step, radius · 0.5)`. Єдиний інваріант `radius_min >= 4 × grid.step` валідується в `ConfigManager` (дефолти: `64 >= 4 × 16`). |
...
| **Sodium** | Краш або чорний екран | ✅ Ризик майже зник разом із власним draw: мод не створює ні пайплайна, ні таргета, ні draw-виклику. Лишається одне — якщо Sodium сам замінює `WeatherEffectRenderer`, наш `renderInstances` просто не викличеться і нахилу не буде. Про це кажемо в лог і в чат, решта працює. |
