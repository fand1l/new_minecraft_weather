...
### Чого немає: хмарність як видима вісь

Вісь `clouds` існує в моделі, в мережі, в командах і в інваріантах — і **ніде не малюється**.
У грі при `set clouds clear` і `set clouds overcast` небо виглядає однаково: ванільні хмари
не мають поняття щільності, це один шар із фіксованим виглядом, і мод його не чіпає.

Що з цієї осі працює вже зараз, попри відсутність рендера: вона тримає інваріанти 1 і 2
(опади й гроза вимагають OVERCAST), керує двофазним переходом (спершу гасне дощ, потім
розходяться хмари) і через `getRainLevel` бере участь у затемненні неба.

Що потрібно, щоб вона стала видимою: міксин на ванільний рендер хмар, який вимикає шар при
CLEAR і лишає при OVERCAST. Це нова ціль, назв класу 26.2 для неї я не читав — тобто спершу
`find-class.sh`, і лише потім код.
